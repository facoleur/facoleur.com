...
export default function Home() {
  ...
  return (
    <div className="prose mx-auto grid w-full grid-cols-1 lg:grid-cols-[1fr_4fr_1fr]">
      <div></div>
      <div className="space-y-24">
        ...
        <section>
          <p>{t("homepage.logos.title")}</p>
          <div className="marquee-wrapper">
            <div className="marquee grid grid-cols-6 gap-8">
              {Array.from({ length: 6 }).map((_, i) => (
                <img
                  key={`a-${i}`}
                  src={`/clientsLogo/${i + 1}.png`}
                  alt={`client ${i + 1}`}
                  className="inline-block h-12 w-auto grayscale transition hover:grayscale-0"
                />
              ))}
            </div>
          </div>
        ...
        <section>
          ...
          <div className="grid gap-1 md:grid-cols-2">
            {t.raw("homepage.packages.items").map((pkg) => (
              <div
                key={pkg.name}
                className="flex h-full flex-col bg-slate-200/60 p-6"
              >
                <div>
...
const Projects = () => {
  ...
  return (
    <div className="group relative flex flex-row gap-8 bg-slate-200/60 p-12">
      <div className="relative w-5/11 overflow-visible">
        {/* images layer */}
        <div className="pointer-events-none absolute top-[-80px] z-10 h-[220px] w-[320px]">
          <div className="relative h-full w-full">
            <img
              src="homepage/av.png"
              className="absolute top-8 left-5 h-40 w-56 rounded shadow-lg transition-transform duration-150 group-hover:-translate-x-2 group-hover:-translate-y-8 group-hover:-rotate-3"
            />
            <img
              src="homepage/te.png"
              className="absolute top-6 left-10 h-40 w-56 rounded shadow-lg transition-transform duration-150 group-hover:-translate-y-8"
            />
            <img
              src="homepage/mp.png"
              className="group:hover:translate-x-2 absolute top-12 left-16 h-40 w-56 rounded shadow-lg transition-transform duration-150 group-hover:translate-x-2 group-hover:-translate-y-8 group-hover:rotate-4"
            />
          </div>
        </div>
        ...
        <div className="relative z-20">
          <svg viewBox="0 0 1657 878" xmlns="http://www.w3.org/2000/svg">
            <path
      ...
      <div>
        <h2>{t("homepage.completed.title")}</h2>
